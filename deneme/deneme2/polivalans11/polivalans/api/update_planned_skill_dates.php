...
try {
    ...
    $checkSQL = "SELECT ps.id 
                 ...
                 JOIN organization_skills os ON ps.skill_id = os.id
                 WHERE p.name = ? AND o.name = ? AND os.skill_name = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $skillSQL = "SELECT id FROM organization_skills WHERE skill_name = ? AND organization_id = ?";
