...
try {
    ...
    // skills tablosundan skill_name'e göre skill_id çek
    $stmtLesson = $pdo->prepare("SELECT id FROM skills WHERE skill_name = :skillName AND is_active = 1 LIMIT 1");
    $stmtLesson->execute(['skillName' => $skill_name]);
    $lesson = $stmtLesson->fetch();

    if (!$lesson) {
        throw new Exception("Seçilen ders skills tablosunda bulunamadı.");
    }
